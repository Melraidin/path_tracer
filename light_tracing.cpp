...
public:
    ...
    void set_photon_map(void* inc_kd) {
    // void set_photon_map(vector<PhotonHit> photon_map) {
        // this->photon_hits = photon_map;
        // for (int i = 0; i < this->photon_hits.size(); i++ ) {
        //     PhotonHit* hit = &this->photon_hits[i];
        //     kd_insert3(this->kd, hit->point.x, hit->point.y, hit->point.z, hit);
        // }
    ...
    vector<PhotonHit> find_nearest_photons(V3 point, int count) {
        ...
        struct kdres *results = kd_nearest_range(this->kd, pt, SEARCH_DISTANCE);
        // cout << "Found " << kd_res_size(results) << " photons\n";

        while (!kd_res_end(results)) {
            /* get the data and position of the current result item */
            double pos[3];
            PhotonHit* pch = (PhotonHit*)kd_res_item(results, pos);
            ...
            PhotonHit hit = PhotonHit(pch->point, pch->flux, pch->incoming);

            hit.distance = sqrt(point.power_distance(hit.point));
    ...
    vector<PhotonHit*> find_nearest_photons_old(V3 point, int count) {
    ...
    ~Renderer(void) {
        // TODO BUGBUG Memory leak.
        // for (int i = 0; i < this->photon_hits.size(); i++) {
        //     delete &this->photon_hits[i];
        // }
        delete[] buffer;
        // kd_free(this->kd);
    }
    ...
    int trace_photon(Ray ray, int hit_count, V3* flux, V3* hit_point) {
        ...
        PhotonHit *phit = new PhotonHit(point, *flux, ray.direction);
        this->photon_hits.push_back(*phit);
        kd_insert3(this->kd, point.x, point.y, point.z, phit);
...
void worker(int worker_num, int photon_count, int iterations, Scene* scene, V4* buffer, double photon_filter, void* kd, int start_row, int row_count) {
    Renderer renderer = Renderer(*scene, photon_filter);

    renderer.set_photon_map(kd);
...
int main(int argc, const char* argv[]) {
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        threads[i] = new boost::thread(boost::bind(&worker, i, photon_count, (double)iterations / (double)thread_count, &scene, buffer, photon_filter, kd, worker_start_row, worker_rows));
    }
