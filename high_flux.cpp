    ...
    void iterate() {
        ...
        for (double y = unifRand() / (double)scene.height, ystep = 1.0L / (double)scene.height;
            ...
            for (double x = unifRand() / (double)scene.width, xstep = 1.0L / (double)scene.width;
                ...
                vector <V3> shoot_hits;
                V3 shoot_color = trace(ray, 0, &shoot_hits);

                V3 final_color = V3();

                for (int shoot_i = 0; shoot_i < shoot_hits.size(); shoot_i++) {
                    for (int gather_i = 0; gather_i < gather_hits.size(); gather_i++) {
                        Ray bi_ray = Ray(
                            shoot_hits[shoot_i],
                            gather_hits[gather_i].sub(shoot_hits[shoot_i]).normalize());
                        ...
                        final_color.iadd(
                            light->material->emission.mul(
                                hit->material->color).add(hit->material->emission));
                    }
                }

                // TODO Change this to the combination of the gather_color and shoot_color;
                //buffer[i++].iadd(gather_color);
                buffer[i++].iadd(final_color.divs((shoot_hits.size() + 1) * (gather_hits.size() + 1)));
            }
        }
    }

    V3 trace(Ray ray, int n, vector <V3>* hits) {
        if (n > 4) {
            return V3();
        }
